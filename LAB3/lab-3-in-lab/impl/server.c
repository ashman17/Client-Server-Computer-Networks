...
void make_reply(char* msg, char* reply) {
	if (strlen(msg) < 5) {
	...
	if (token) {
		...
		if (num > len) {
			sprintf(reply, "%s (%d bytes sent)", reply, len);
		} 
		else if (num<0) {
			char* temp = (char*) malloc(sizeof(reply));
			strcpy(temp, reply);
			strcpy(reply, temp+len+num);
		}
		else {
			reply[num] = '\0';
		}
